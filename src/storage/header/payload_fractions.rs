...
impl ParseBytes for MaximumEmbeddedPayloadFraction {
  ...
  fn parsing_handler(bytes: &[u8]) -> SqliteResult<Self> {
    let maximum = *bytes.first().ok_or(field_parsing_error! {Self::NAME.into()})?;
    if maximum == 64 {
      Ok(Self(maximum))
    } else {
...
impl ParseBytes for MinimumEmbeddedPayloadFraction {
  ...
  fn parsing_handler(bytes: &[u8]) -> SqliteResult<Self> {
    let minimum = *bytes.first().ok_or(field_parsing_error! {Self::NAME.into()})?;
    if minimum == 32 {
      Ok(Self(minimum))
    } else {
      Err(SqliteError::Custom(
...
impl ParseBytes for LeafPayloadFraction {
  ...
  fn parsing_handler(bytes: &[u8]) -> SqliteResult<Self> {
    let leaf = *bytes.first().ok_or(field_parsing_error! {Self::NAME.into()})?;
    if leaf == 32 {
      Ok(Self(leaf))
    } else {
      Err(SqliteError::Custom("LeafPayloadFraction must be 32.".into()))
    }
  }
}
