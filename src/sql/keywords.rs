//! # SQLite Keywords
//!
//!  The SQL standard specifies a large number of keywords which may not be used
...
//! keywords is so long that few people can remember them all. For most SQL
//! code, your safest bet is to never use any English language word as the name
...
//! then the token is understood to be an identifier instead of a string
//! literal.
//!
...
//! normally quote any identifier that is an English language word, even if you
//! do not have to.
//!
//!  The list below shows all possible keywords used by any build of SQLite
//! regardless of compile-time options. Most reasonable configurations use most
//! or all of these keywords, but some keywords may be omitted when SQL language
//! features are disabled. Applications can use the sqlite3_keyword_count(),
//! sqlite3_keyword_name(), and sqlite3_keyword_check() interfaces to determine
//! the keywords recognized by SQLite at run-time. Regardless of the
...
//!
pub static SQLITE_KEYWORDS: [&'static str; 147] = [
  "ABORT",
  "ACTION",
  "ADD",
  "AFTER",
